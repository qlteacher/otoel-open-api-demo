...
public class Process2Demo {
    ...
    public static void main(String[] args) {
        ...
        //加载添加课例数据模板
        File file = Constant.getFile(String.format("test-%s-%s.template", Constant.activityId, ConfigUtil.getConfig().getLesson().getCatalogId()));
        //判断文件是否存在
        if(file.exists()){
            ...
            //指定生成提交用json数据的保存位置
            File outFile = Constant.getFile(String.format("test-%s-%s-submit.json", Constant.activityId, ConfigUtil.getConfig().getLesson().getCatalogId()));
