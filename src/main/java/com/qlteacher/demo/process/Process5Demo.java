...
public class Process5Demo {
    ...
    public static void main(String[] args) {
        ...
        File jpg = new File(classLoader.getResource("file/big-cover.jpg").getFile());
        int i =0;
        for (UploadLessonItemVO item : itemVOS) {
            ...
            } else if (item.getLimit().stream().anyMatch(limit -> limit.getFileFormat().contains("jpg"))) {
                UploadLessonStructureResourceDemo.uploadLessonStructureResource(item.getLessonCaseId(), item.getLessonCaseStructureId(), jpg);
            }  else {
